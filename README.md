...
| `activation-timeout` | *(none)* | How long a test process may spend activating its environment — which includes its own precompilation — before the run gives up on it and errors that environment's test items. Unset by default: a slow activation is legitimate, and this bounds a stalled one, which nothing else does. |
| `run-stall` | *(none)* | Opt-in: error the run's remaining test items after this many seconds with no test process busy and no message about it. Activating, revising and running an item all count as progress, so this bounds a run whose workers died without reporting it, not a slow one. Worth setting in CI, where a hung run otherwise burns the whole job timeout. |
...
| `max-workers` | *(CPU threads, capped)* | Maximum number of parallel test processes. Default: the number of CPU threads, at most 8 and at most one per 3 GiB of system memory. |
...
| `gc-between-testitems` | *(off)* | `true` or `false`; unset leaves the default, which is off. `true` runs a full garbage collection after every test item. This slows runs down, and on machines short of memory it slows them down a lot. It can help when test items hold memory outside the Julia heap that only finalizers release. |
| `memory-threshold` | *(off)* | Recycle a test process once its own resident memory exceeds this fraction (0–1) of total system memory. Experimental. |
